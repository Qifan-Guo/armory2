...
# from armory2.
def test(request):
    data=Gowitness_Data.objects.all()
    return render(request,'gowitnessPage/test.html',{'data': data})

def sort_status_code(request):
    data=Gowitness_Data.objects.all().order_by("response_code_string")
    return HttpResponseRedirect(reverse("dd", kwargs={'data':data}))
...
def index(request):
    ...
    #request.session['gowitness_data']= data2
    data=Gowitness_Data.objects.all().order_by("response_code_string")
