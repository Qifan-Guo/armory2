...
from armory2.armory_main.models import IPAddress, Domain,Gowitness_Data
...
from armory2.armory2.settings import STATIC_URL
import os
import re
import subprocess
import tempfile
...
class Module(ToolTemplate):
    ...
    def build_cmd(self, args):
        ...
        command = (
            self.binary + " file -f {target} -D {output}/gowitness.db -P armory_main/static/gowitness"
        )
    ...
    def process_output(self, cmds):
        ...
        for cmd in cmds:
            ...
            for u in cr.execute('select id, url, filename, final_url, response_code, url, title from urls').fetchall():
                ...
                if not port:
                    ...
                    data = {
                        ...
                        'initial_url': u[5],
                        'title':u[6],
                        'technologies': {'technologies':[ k[0] for k in cr.execute('select value from technologies where url_id = ?', (u[0],))]},
                        }
                    gowitness_data=Gowitness_Data.objects.create(**data)
                    gowitness_data.save()
